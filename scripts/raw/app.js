var app = angular.module('myApp', ['ngRoute', 'ipCookie', 'ui.sortable']);
...
app.directive('syncFocusWith',["$timeout", "$rootScope", function($timeout, $rootScope) {
    return {
        restrict: 'A',
        scope: {
...
/* Panel resizer */
app.directive('resizer', ["$document", "ipCookie",function($document, ipCookie) {
...
app.factory("jsonDefer",["$http", "$q",function($http, $q){
...
app.controller('ArabicKeyCtrl',["$scope", "$http","ipCookie","jsonDefer",function($scope, $http, ipCookie, jsonDefer) {
	$scope.text = "";
	$scope.options = [];
	$scope.cookieExpire = 2000;
	$scope.transliterationScheme = $scope.options['transliterationScheme'] = "Kamal";
	
	$scope.transliterationJson = ["json/Custom.json","json/Kamal.json", "json/ArabTex.json", "json/Indo.json"];
	...
	$scope.initCookie("transliterationScheme", "Custom");
